...
for idx in data:
    if idx.endswith('json'):
        json_file_paths.append(os.path.join('data', idx))
json_file_paths

stars_paths = []
for idx in data:
    if idx.startswith('stars'):
        stars_paths.append(os.path.join('data', idx))
stars_paths
...
data = os.listdir('data')
broken_data = os.listdir('broken_data')
...
for planets in planets_list:
    major_radius = planets.semi_major_radius
    period = planets.orbital_period
    names = planets.host_name
    if major_radius == None:
        continue
    if period == None:
        continue
    if names == "GJ 9827":
        ratios_gj9827.append(period**2 / major_radius**3)
...
value1 = statistics.mean(ratios_gj9827)
value2 = statistics.stdev(ratios_gj9827)
coeff_gj9827 = value2 / value1
coeff_gj9827
...
predicted_mass = 133408 / value1
actual_mass = stars_dict["GJ 9827"][3]
...
flux_list = []
temp_4th_power_list = []
for tuples in planets_list:
    flux = tuples.insolation_flux
    temp = tuples.equilibrium_temperature
    if flux == None or temp == None:
        continue
    temp = temp**4
    flux_list.append(flux)
    temp_4th_power_list.append(temp)
...
for values in planets_list:
    temp2 = values.equilibrium_temperature
    flux2 = values.insolation_flux
    if flux2 == None or temp2 == None:
        continue
    if values.insolation_flux > 7000:
        continue
    temp2 = temp2**4
    temp_4th_power_list_no_outliers.append(temp2)
    flux_list_no_outliers.append(flux2)
...
for mass in stars_dict:
    star_mass = stars_dict[mass].stellar_mass
    objects = stars_dict[mass]
    if star_mass == None:
        continue
    if star_mass < 0.3:
        continue
    
    if star_mass >= 0.3 and star_mass < 8:
        star_classes['Red Giant'].append(objects)
    if star_mass >= 8 and star_mass < 10.5:
        star_classes["White Dwarf"].append(objects)
    if star_mass > 10.5:
        star_classes['Neutron Star'].append(objects)
...
for star_class in star_classes:
    star_list = star_classes[star_class]
    empty_list = []
    for star in star_list:
        light = star.stellar_luminosity
        if light == None:
            continue
        empty_list.append(light)
    light_avg = statistics.mean(empty_list)
    star_classes_avg_lum[star_class] = light_avg
star_classes_avg_lum
...
for star_class in star_classes:
    star_list = star_classes[star_class]
    for star in star_list:
        light2 = star.stellar_luminosity
        mass =star.stellar_mass
        radii = star.stellar_radius
        if radii == None:
            continue
        if mass == None:
            continue
        ...
        lum_dict[star_class].append(light2)
...
for star_class in star_classes:
    ...
    for star in star_list:
        ...
        lum_dict_no_outliers[star_class].append(light2)
...
age_list = []
mass_list = []
for objects in stars_dict:
    light3 = stars_dict[objects].stellar_luminosity
    temp = stars_dict[objects].stellar_effective_temperature
    age = stars_dict[objects].stellar_age
    mass = stars_dict[objects].stellar_mass
    if light3 ==None:
        continue
    if age == None:
        continue
...
def get_all_paths_in(directory):
    ...
    for file in os.listdir(directory):
        ...
        if os.path.isdir(path):
            ...
            thing.extend(get_all_paths_in(path))
        else:
            thing.append(path)
    thing.sort()
...
for paths in broken_data:
    all_planets_list.extend(get_planets(os.path.join('data','planets_5.csv'),paths))
...
def get_distances_to_star(planet):
    if planet.semi_major_radius == None or planet.eccentricity == None:
        ...
        distance_list = []
        short_dis = planet.semi_major_radius * (1 - abs(planet.eccentricity))
        long_dis = planet.semi_major_radius * (1 + abs(planet.eccentricity))
        distance_list.append(short_dis)
        distance_list.append(long_dis)
...
def get_liquid_water_distances(planet):
    ...
    star = stars_dict[host_star] 
    if star.stellar_luminosity == None:
        return None
    else:
        h20_list = []
        h20_short_dis = ((10**star.stellar_luminosity)/1.15)**0.5
        h20_long_dis = ((10**star.stellar_luminosity)/0.53)**0.5
        h20_list.append(h20_short_dis)
        h20_list.append(h20_long_dis)
    return h20_list
#         stell_h20_long_dis
#         stell_h20_short_dis
...
for planet in all_planets_list:
    ...
    liquid = get_liquid_water_distances(planet)
    if distance[0] > liquid[0] and distance[1] < liquid[1]:
        planets_with_liquid_water.append(names)
...
def get_surface_temperatures(planet):
    if planet.equilibrium_temperature == None:
        ...
        surface_temp = []
        min_surf_temp = ((1-0.5)**0.25)*planet.equilibrium_temperature
        max_surf_temp = ((1-0.0)**0.25)*planet.equilibrium_temperature
        surface_temp.append(min_surf_temp)
        surface_temp.append(max_surf_temp)
    return surface_temp
...
for planet in all_planets_list:
    ...
    test = get_surface_temperatures(planet)
    if test == None:
        continue
    if test[0] > 263 and test[1] < 323:
...
for planet in all_planets_list:
    ...
    if temp == None:
        continue
    if gravity > 0.75 and gravity < 1.25 and distance[0] > water[0] and distance[1] < water[1] and \
    temp[0] > 200 and temp[1] < 350:
        habitable_planets.append(name2)
habitable_planets
